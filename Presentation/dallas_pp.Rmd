...
```{r ggplot_frq_crime, message=FALSE, warning=FALSE}
...
```{r dallas_gender_and_timeslot, message=FALSE, warning=FALSE}
...
```{r dallas_crime_and_timeslot,message=FALSE, warning=FALSE}
...
```{r dallas_crime_rate_per_week_and_timeslot,message=FALSE, warning=FALSE}
...
### Not feasible 
```{r Hate Crime Analysis, message=FALSE, warning=FALSE}
ggplot_hc <-dallas_incidents%>%
select(`Hate Crime Description`)%>%
group_by(`Hate Crime Description`)%>%
summarize( `number of crimes` = n())%>%
ggplot(mapping = aes(x=`number of crimes`,y=`Hate Crime Description`))  +
  geom_point()+labs(x = "Number of Crimes",y="Hate Crime Type")
ggplot_hc
```
...
```{r age group,message=FALSE, warning=FALSE}
...
summarize( `number of crimes` = n())%>%
mutate(percentage = sprintf("%.1f%%",(`number of crimes` / sum(`number of crimes`))*100))%>%
ggplot(mapping = aes(x=`age group`,y=`number of crimes`,label=percentage, fill = `age group`))  +
...
```{r dallas_crime_rate_per_week,message=FALSE, warning=FALSE}
