...
## 3.7 Heatmap
### Generation of a line graph based on the crimes and corresponding time slots using which we generate a heat map with the lowest number of crimes being depicted by white and the highest by red. 
...
ggplot(data = dallastw, aes(x= `time slot of occurence`, y = `number of crimes`, fill = `week of the day`)) + geom_line(aes(group = `week of the day` , color = `week of the day`)) + coord_flip() + labs(x = "Time of occurence",y="Number of Crimes",title="Crimes and corresponding time slot of occurence ")
```
...
```
### 4.1.2 Following chunk performs evaluation of variable importance using violinplot visualizations and anova method
